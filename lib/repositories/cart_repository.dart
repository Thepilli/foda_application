...
class CartRepository {
  ...
  Future<Either<ErrorHandler, CartItem>> getCartItem(String uid, String foodId) async {
    try {
      ...
      try {
        final cartSnapshot = await _firestoreUser.doc(uid).collection('cart').doc(foodId).get();
        if (cartSnapshot.exists) {
          fodaPrint(cartSnapshot.data());
          return Right(CartItem.fromMap(cartSnapshot.data() as Map<String, dynamic>));
        } else {
          return const Left(ErrorHandler(message: 'Failed to get the cartItem'));
        }
